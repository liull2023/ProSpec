...
from OWN_linear import OWNLinear
...
class CycDM(nn.Module):
    ...
    def do_forward_predict(self, first_latent, target_latents, observation,
        ...
        forward_pred_rewards.append(pred_rew)
        for j in range(1, self.jumps + 1):
            latent = self.inverible_model(torch.cat([latent, action[j - 1]], dim=1), invert=False)
            latent = self.OWN(latent)
    ...
    def do_backward_predict(
        ...
        for j in reversed(range(1, 1 + self.jumps)):
            latent = self.OWN(backward_pred_latents[-1],invert=True)
            latent = self.inverible_model(latent, invert=True)
    ...
    def do_cycle_predict(
        ...
        for i in range(1, 1 + self.cycle_steps):
            latent = self.inverible_model(torch.cat([forward_pred_latents[-1], forward_actions[-1]], dim=1), invert=False)
            latent = self.OWN(latent)
            forward_pred_latents.append(latent)
    ...
    def do_real_cycle_loss(
        ...
        backward_pred_latents = [latent]
        for i in reversed(range(1, 1 + self.cycle_steps)):
            latent = self.OWN(torch.cat([backward_pred_latents[-1],
                           forward_actions.pop()], dim=1), invert=True)
            latent = self.inverible_model(latent, invert=True)
            backward_pred_latents.append(latent)
            cycle_latent = latent
    ...
    def cal_Q_value(self, latents, actions):
        all_latents = torch.cat([l.view(-1, l.size(-1)) for l in latents], dim=0)
        all_actions = torch.cat([a.view(-1, a.size(-1)) for a in actions], dim=0)
        with torch.no_grad():
            Q1, Q2 = self.critic_target(all_latents, all_actions)
        K, B, _ = latents[0].shape
        D = len(latents)
        Q1 = Q1.view(K, B, D)
        Q2 = Q2.view(K, B, D)
        Q = torch.min(Q1,Q2)
    ...
    def search_opt_at(self, first_actions, rewards, weight=0.25):
        scores = rewards
        best_idx = scores.argmax(dim=0).unsqueeze(0) 
        idx_expanded = best_idx.expand(first_actions.size(0), *first_actions.shape[1:])
        best_action = torch.gather(first_actions, 0, idx_expanded)[0]
        return best_action  

    def calculate_discounted_cumulative_reward(self, rewards, gamma):
        if not isinstance(rewards, torch.Tensor):
            # list of (K, B, …)  -->  Tensor of shape (K, B, T, …)
            rewards = torch.stack(rewards, dim=2)
        T = rewards.size(2)
        idx = torch.arange(T - 1, -1, -1, device=rewards.device, dtype=rewards.dtype)
        discount = gamma ** idx
        discount_shape = [1] * rewards.dim()
        discount_shape[2] = T
        discount = discount.view(*discount_shape)
        discounted = rewards * discount
        return discounted.sum(dim=2)
     
    def opt_at_predict(self, observation, action, iteraction, future_step):
        K = iteraction  

        with torch.no_grad():
            aug_obs = self.transform(
                observation[None].expand(1, *observation.shape).flatten(0,1),
                augment=True
            )
            cycle_target = self.target_encoder(aug_obs).detach()
            cycle_target = cycle_target.unsqueeze(0) 
            latents = cycle_target.repeat(K, 1, 1) 
            obs_batch = observation.unsqueeze(0).expand(K, *observation.shape)

            K, B1, C, H, W = obs_batch.shape
            if B1 > 1:
                print(1)
            x = obs_batch.view(K * B1, C, H, W)  
            _, actions,_,_ = self.actor(x, compute_log_pi=False)
            actions = actions.view(K, B1,-1)    
            actions[0] = action
            forward_pred_latents  = [latents]
            forward_pred_actions  = [actions]
            for _ in range(future_step):
                inp = torch.cat([forward_pred_latents[-1], forward_pred_actions[-1]], dim=-1) 
                K1, B2, C1 = inp.shape
                inpx = inp.view(K1 * B2, C1)   
                latents = self.inverible_model(inpx, invert=False)
                latents = self.OWN(latents)
                

                actions, _,_,_ = self.actor(latents, compute_log_pi=False, compute_pi=False)
                latents = latents.view(K1, B2, -1) 
                forward_pred_latents.append(latents)
                actions = actions.view(K, B1, -1) 
                forward_pred_actions.append(actions)
            Q_values = self.cal_Q_value(forward_pred_latents, forward_pred_actions)  
            cumrew = self.calculate_discounted_cumulative_reward(Q_values, gamma=0.99)

            best_action = self.search_opt_at(forward_pred_actions[0],cumrew)
            return best_action.view(action.shape)
...
class CycDMSacAgent(object):
    ...
    def select_action(self, obs, reflection, step, future_step):
        with torch.no_grad():
            if self.training:
            ...
            elif not reflection or step<self.reflection_step:
                obs = torch.FloatTensor(obs).to(self.device)
                obs = obs.unsqueeze(0)
                mu, _, _, _ = self.actor(obs,
                                        compute_pi=False,
                ...
                mu, _, _, _ = self.actor(obs,
                                        compute_pi=False,
                                        compute_log_pi=False)
                mu = self.CycDM.opt_at_predict(obs, mu, self.iteraction, future_step)
                
        return mu.cpu().data.numpy().flatten()

    def sample_action(self, obs, reflection, step, future_step, reflection_step):
    ...
    def update_cycdm(self, cycdm_kwargs, L, step):
        ...
        input_obs = observation[0].flatten(1, 2)# 128,9,9,100
    ...
    def update(self, replay_buffer, L, step):
        if self.encoder_type == 'pixel':
            ...
            obs, action, reward, next_obs, not_done, cycdm_kwargs, idxs = elements
        else:
