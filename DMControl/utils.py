...
class ReplayBuffer(Dataset):
    ...
    def sample_spr(self):   # sample batch for auxiliary task
        ...
        spr_samples = {
            'observation': obses.transpose(0, 1).unsqueeze(3),# (jumps+1, B, 1, 3, 100, 100)
            'action': actions.transpose(0, 1),# (jumps+1, B, action_shape)
            'reward': rewards.transpose(0, 1),# (jumps+1, B, 1)
        }
        return (*self.sample_aug(original_augment=True), spr_samples, idxs)
